...
def _become_dpi_aware():
    """Opt this process into per-monitor DPI awareness BEFORE any window query.

    Without this, a DPI-unaware process is lied to by Windows: on a 150% display
    GetClientRect reports the DPI-VIRTUALISED size (1280x960 -> 853x640), and
    PrintWindow then renders into a bitmap that small while the app keeps
    drawing at its true physical resolution. The result is not a scaled-down
    capture, it is a TOP-LEFT CROP — the right ~33% and bottom ~33% of the
    window are simply missing from the file.

    That is worse than an obviously broken capture, because a cropped shot looks
    like a valid screenshot of a UI whose right-hand side is clipped. On
    2026-08-16 it made the F10 menu's GAME dropdown appear to overflow the
    window (hints and values cut off mid-word) when the real 1280x960 window was
    drawing all of it with room to spare.

    Per-monitor v2 first (Win10 1703+), falling back to the older APIs. """
    try:
    ...
    try:
        ...
        pass
    try:
        user32.SetProcessDPIAware()                      # system-DPI aware
    except AttributeError:
        pass
