...
- [ ] LWL/LWR/SWL/SWR unaligned — psx-spx "CPU Load/Store"; Amidog.
  - [x] **LWL/LWR merge operand vs the load delay — FIXED 2026-08-15.** LWL/LWR
    take rt as both input (merge base) and output, and read that input late
    enough to receive the forwarded result of an immediately preceding load —
    so they are NOT subject to the load delay. `code_generator.cpp` deferred a
    load's writeback into `psx_ldd_<addr>` whenever the successor read the same
    register, then emitted the successor LWL/LWR merging into the STALE
    `cpu->gpr[rt]` and discarded the temp ("successor write wins"). The
    producer-side case was already handled (`full_function_emitter.cpp`
    "complementary" LWL+LWR pairs); the consumer-side case — a plain `LW`
    feeding an `LWR` — fell through to the generic path. Fix: the pair emitter
    now points the merge operand at the deferred temporary
    (`set_lwlr_merge_forward` / `lwlr_merge_operand`).
    Found via Yu-Gi-Oh! Forbidden Memories (SLUS-01411), whose model renderer
    uses `lw $t0,12($a3)` + `lwr $t0,10($a3)` to assemble a vertex index from
    two non-adjacent halfwords. The stale merge made the index leftover bits,
    indexing unfilled vertex-cache slots holding the pool default
    `{sxy=0x10001000, sz=0x1000}`; masked to the GPU's 11-bit coords that is
    (0,0), so quads drew three vertices on the model and the fourth at the
    screen origin — long spikes. ALL 16 load-delay pairs in that title's
    3.4M-line output had an LWL/LWR successor, i.e. every one was wrong.
    Verified: sentinel vertices 16-18/20 → 0, oversize polys 6 → 0, with the
    quads still drawn (not culled).
...
- [ ] **SPU**: 24 voices, ADSR, pitch/sample-rate, reverb, volume sweeps, IRQ —
  ...
  - [~] **KON/KOFF are not tick-latched — PARTIALLY FIXED 2026-08-16.**
    Hardware latches KON (1F801D88) and KOFF (1F801D8C) and processes them
    together once per 44100Hz cycle; a voice named by BOTH in one cycle ends up
    PLAYING (key-on wins). `spu.c` applied each register write immediately, so
    CPU-side write order within a frame became audible. Yu-Gi-Oh! Forbidden
    Memories' menu SFX writes `KOFF v; KON v; KOFF v; KOFF v` in one frame and
    expects the sound; the trailing KOFF won, dropping the voice into RELEASE,
    and because those one-shots use ADSR = 0x00000000 (release shift 0) release
    is instantaneous — silent SFX. Intermittent because it depended on whether
    a render landed between the KON and the trailing KOFF.
    Mitigation applied: `kon_this_block` makes a KOFF unable to cancel a KON
    issued in the same render window, i.e. same-cycle key-on priority at the
    granularity `spu_render` offers. Writes still apply immediately, so ENDX
    clearing on KON keeps its timing.
    **Still open:** the faithful model latches both and processes them at the
    44100Hz tick. Not done because `spu_render` works in ~735-sample blocks, so
    deferring KON would move the ENDX clear ~16ms later than hardware's ~22us
    and risk breaking games that poll ENDX right after keying on. Doing this
    properly needs KON/KOFF evaluated on the guest-clock SPU timeline (see the
    "port Beetle PS_SPU on a guest-clock timeline" recommendation below).
    **Unverified:** that key-on wins same-cycle is inferred from this game
    working on real hardware, not from psx-spx or an oracle run. Confirm before
    upstreaming.
