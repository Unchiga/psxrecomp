...
void gpu_texture_correction_stats(uint32_t *attempts, uint32_t *no_source,
...
static void prepare_texture_triangle(int i0, int i1, int i2) {
    gr_set_perspective_triangle(0, 0.0f, 0.0f, 0.0f);
    if (!s_texture_correction_enabled) return;
    s_persp_attempts++;
    if (gp0_cmd_source_addr == 0xFFFFFFFFu) { s_persp_no_src++; return; }
