...
int gl_renderer_interp_readback(int which, uint32_t *out, int pitch) {
    if (!s_ctx || !s_raster_ok || !out || !s_interp_mutex) return 0;
    SDL_LockMutex(s_interp_mutex);
    const int w = s_interp_w, h = s_interp_h;
    const int S = s_scale > 0 ? s_scale : 1;
    if (s_interp_valid < 1 || w <= 0 || h <= 0) { SDL_UnlockMutex(s_interp_mutex); return 0; }
    if (which == 1 && s_interp_valid < 1) { SDL_UnlockMutex(s_interp_mutex); return 0; }
    if (which == 0 && s_interp_valid < 2) { SDL_UnlockMutex(s_interp_mutex); return 0; }

    uint8_t *rgba = (uint8_t *)malloc((size_t)w * h * 4u);
    if (!rgba) { SDL_UnlockMutex(s_interp_mutex); return 0; }
    ...
    if (which == 2) {
        ...
        if (s_interp_fence[idx])
            p_glWaitSync(s_interp_fence[idx], 0, PSXGL_TIMEOUT_IGNORED);
...
/* Is the presentation thread demonstrably still presenting?
 *
 * The main thread hands its Swap to that thread whenever a history pair
 * exists. Doing so on the ASSUMPTION the other thread will make the Swap is
 * only safe with a liveness check: on 2026-08-16 the presenter parked in an
 * un-signallable wait (shared auto-reset waitable timer, since fixed) and the
 * window froze for 92 seconds while the guest ran on at 240 fps and
 * interp_capture kept succeeding 286 times a second. Nothing in the
 * interpolation state said anything was wrong — enabled, not suspended, a
 * valid pair — because the fault was in a thread, not in the state.
 *
 * A stalled presenter now costs one main-thread present instead of the whole
 * output. Reads s_interp_swaps unlocked: it is a monotonically increasing
 * aligned 64-bit counter, so a stale read only delays the fallback by a frame.
 */
#define INTERP_PRESENT_STALL_MS 250u
static uint64_t s_interp_last_swap_ms;
static uint64_t s_interp_seen_swaps;

static int interp_presenter_alive(void) {
    uint64_t now = SDL_GetTicks64();
    if (s_interp_swaps != s_interp_seen_swaps) {
        s_interp_seen_swaps = s_interp_swaps;
        s_interp_last_swap_ms = now;
        return 1;
    }
    /* First pair of a session: give it the same grace as a live presenter
     * rather than declaring it dead before it has had a chance to run. */
    if (s_interp_last_swap_ms == 0u) {
        s_interp_last_swap_ms = now;
        return 1;
    }
    return (now - s_interp_last_swap_ms) < INTERP_PRESENT_STALL_MS;
}
...
void gl_renderer_present_vram(int disp_x, int disp_y, int w, int h, int linear,
    ...
    flush_tex_batch();
    flush_cpu_upload();
    /* BEFORE the skip test, not inside the interp branch below. The presenting
     * thread only ever READS this canvas (it may not redraw it — that would
     * race the emu thread's ownership of the menu state), so the emu thread has
     * to refresh it even on frames where the game itself presents nothing. A
     * static screen skips the present entirely, and with the refresh sited
     * inside the interp branch the menu then froze at whatever it last drew
    ...
    if (interp_pair && interp_presenter_alive()) {
