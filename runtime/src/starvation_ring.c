...
void starvation_watchdog_check(void) {
    ...
    /* Snapshot ONCE. The heartbeat is written by the debug-server IO thread
     * while this check runs on the emu thread, so re-reading the global inside
     * the comparison and the message could observe two different values. */
    const uint64_t last = s_last_heartbeat_us;
    /* A heartbeat at or AHEAD of `now` means it landed between this thread's
     * clock read and this load — the runtime is alive, not starved. Both are
     * uint64, so subtracting in that order wraps: on 2026-08-16 a 13 us skew
     * became 18446744073709551603 us "without heartbeat" and aborted a healthy
     * session mid-duel. A real stall still trips, because then last < now. */
    if (last >= now) return;
    if (now - last > timeout) {
