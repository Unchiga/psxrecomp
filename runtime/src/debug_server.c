...
/* Same raw-scan semantics as json_get_int, for fields that are genuinely
 * fractional (blend factors). Accepts bare or quoted decimals. */
...
/* The F10 menu's own view of itself. Presented pixels cannot distinguish an
 * open dropdown from a hover highlight, and say nothing at all when the
 * overlay fails to reach the screen -- so report the state directly. */
static void handle_menu_state(int id, const char *json)
{
    PsxVideoMenuDebug m;
    (void)json;
    psx_video_menu_debug_snapshot(&m);
    send_fmt("{\"id\":%d,\"ok\":true,\"visible\":%d,\"expanded\":%d,"
             "\"menu\":%d,\"item\":%d,\"rows\":%d,"
...
static void handle_input_lag(int id, const char *json)
{
    char *buf;
    if (json_get_int(json, "reset", 0)) {
        psx_host_lag_reset();
        send_fmt("{\"id\":%d,\"ok\":true,\"reset\":true}", id);
        return;
    }
    buf = (char *)malloc(16384);
    if (!buf) { send_err(id, "oom"); return; }
    int n = snprintf(buf, 16384, "{\"id\":%d,\"ok\":true,", id);
    n += psx_host_lag_json(buf + n, 16384 - n);
    snprintf(buf + n, (size_t)(16384 - n), "}");
    send_line(buf);
    free(buf);
}

/* Minimal, fast-to-poll input-latency probe: the current frame, what the HOST
 * can see on the pad right now, and what the GUEST currently reads. Polling
 * this while a button is pressed gives the frame gap between the two, which is
 * the emulator's own contribution to input lag. Kept small deliberately so it
 * can be polled at ~1ms without the response dominating the measurement. */
static void handle_pad_probe(int id, const char *json)
{
    (void)json;
    send_fmt("{\"id\":%d,\"ok\":true,\"frame\":%llu,"
             "\"host0\":\"0x%04X\",\"guest0\":\"0x%04X\"}",
             id, (unsigned long long)s_frame_count,
             (unsigned)psx_host_pad_mask_for_slot(0),
             (unsigned)sio_get_pad_buttons_slot(0));
}

/* Push a host OSD toast. Exists so the OSD can be exercised without waiting
 * for the event that normally raises one (a controller hotplug, a savestate). */
static void handle_osd_toast(int id, const char *json)
{
    char msg[128];
    int ms = json_get_int(json, "ms", 2500);
    if (!json_get_str(json, "msg", msg, sizeof(msg))) {
        send_err(id, "missing msg"); return;
    }
    host_osd_push(msg, ms);
    send_fmt("{\"id\":%d,\"ok\":true,\"msg\":\"%s\",\"ms\":%d,\"needs_present\":%d}",
             id, msg, ms, host_osd_needs_present());
}

/* SDL's joystick/gamepad enumeration + how each player slot is routed. */
static void handle_pad_devices(int id, const char *json)
{
    char *buf = (char *)malloc(8192);
    (void)json;
    if (!buf) { send_err(id, "oom"); return; }
    int n = snprintf(buf, 8192, "{\"id\":%d,\"ok\":true,", id);
    n += psx_host_pad_devices_json(buf + n, 8192 - n);
    snprintf(buf + n, (size_t)(8192 - n), "}");
    send_line(buf);
    free(buf);
}

/* Drive the overlay UI the way a player does. Coordinates are WINDOW pixels
 * (the same units SDL reports), so a test can click a menu title by position. */
static void handle_menu_click(int id, const char *json)
{
    int x = json_get_int(json, "x", -1);
    int y = json_get_int(json, "y", -1);
    if (x < 0 || y < 0) { send_err(id, "missing x/y"); return; }
    if (!psx_host_inject_mouse_click(x, y)) { send_err(id, "push failed"); return; }
    send_fmt("{\"id\":%d,\"ok\":true,\"x\":%d,\"y\":%d}", id, x, y);
}

static void handle_menu_move(int id, const char *json)
{
    int x = json_get_int(json, "x", -1);
    int y = json_get_int(json, "y", -1);
    if (x < 0 || y < 0) { send_err(id, "missing x/y"); return; }
    if (!psx_host_inject_mouse_move(x, y)) { send_err(id, "push failed"); return; }
    send_fmt("{\"id\":%d,\"ok\":true,\"x\":%d,\"y\":%d}", id, x, y);
}
...
static void handle_menu_key(int id, const char *json)
{
    ...
    if (key < 0) { send_err(id, "missing key"); return; }
    if (!psx_host_inject_key(key)) { send_err(id, "push failed"); return; }
    send_fmt("{\"id\":%d,\"ok\":true,\"key\":%d}", id, key);
}

/* ram_dump_file addr=<hex> len=<n> path=<file> — snapshot guest RAM to a FILE.
 *
 * Value scanning ("which word incremented when the player did X") needs whole-
 * RAM snapshots repeatedly. Doing that over the socket is self-defeating: the
 * server is pumped on the main thread, so a 2 MB hex response is emulator time
 * not spent emulating, and the chunked alternative is ~128 round trips per
 * snapshot. On 2026-08-16 a ~100 command/s poll was visible to the player as
 * stutter — an observer that perturbs what it measures.
 *
 * Writing to disk keeps the socket carrying only the acknowledgement, so a full
 * snapshot costs one command. Raw little-endian bytes, no header: the client
 * mmaps/reads it and does the diffing host-side where it is free.
 *
 * Defaults cover all of main RAM (0x80000000, 2 MB). */
static void handle_ram_dump_file(int id, const char *json)
{
...
static void handle_fast_loads(int id, const char *json)
{
...
static void handle_fill_ram(int id, const char *json)
{
    char addr_str[32], val_str[32];
    if (!json_get_str(json, "addr", addr_str, sizeof(addr_str))) {
        send_err(id, "missing addr"); return;
...
/* gl_interp — report the interpolation state, and optionally poke the two
 * live knobs a defect hunt needs without a relaunch (a rebuild costs the
 * player their place in the game):
 *
 *   state_fix=<0|1>  guard that normalises the presentation context's GL state
 *                    before the interpolated draw. The OSD compositor shares
 *                    that context and leaves GL_BLEND armed; `draw_blend` in
 *                    the reply is what the draw actually inherited, sampled
 *                    before the guard runs. 0 reproduces the unguarded draw.
 *   alpha=<-1..1>    pin the crossfade factor. 1 = show the current capture
 *                    only, 0 = the previous one, -1 = follow the frame clock.
 *                    Pinning separates "the blend is wrong" from "the captured
 *                    image is wrong" without reading a single pixel back.
 */
static void handle_gl_interp(int id, const char *json)
{
    int state_fix = json_get_int(json, "state_fix", -1);
    double alpha = json_get_double(json, "alpha", -2.0);
    if (state_fix >= 0 || alpha >= -1.0)
        gl_renderer_interp_set_debug(state_fix, alpha);

    /* Live engage. Interpolation is otherwise selected before the window
     * exists (env / mod / menu_settings.ini), so every A/B used to cost a
     * relaunch — and a relaunch costs the whole intro. It also removes the
     * "did the env var actually reach the process" doubt, which silently
     * invalidated two measurements in this session alone. Serviced on the emu
     * thread, which holds the main GL context: the same thread and the same
     * call the renderer init uses. */
    {
        int enable = json_get_int(json, "enable", -1);
        int fps    = json_get_int(json, "fps", -1);
        if (enable >= 0 || fps >= 0) {
            GlInterpDebug d;
            int en = 0; double hh = 0.0, th = 0.0;
            gl_renderer_interpolation_diag(&en, NULL, NULL, &hh, &th, NULL);
            gl_renderer_interp_debug(&d);
            if (enable < 0) enable = en;
            if (fps < 0) fps = (int)th;   /* keep the current target */
            gl_renderer_set_interpolation(enable, hh, (double)fps, d.blend_mode);
        }
    }
    ...
    send_fmt("{\"id\":%d,\"ok\":true,\"enabled\":%d,\"suspended\":%d,\"history\":%d,"
             ...
             "\"captures\":%llu,\"tex_w\":%d,\"tex_h\":%d,\"scale\":%d,"
             "\"src_x\":%d,\"src_y\":%d,\"src_w\":%d,\"src_h\":%d,"
             "\"source_path\":%d,\"force_4_3\":%d,\"linear\":%d,\"blend_mode\":%d,"
             "\"draw_blend\":%d,\"draw_blend_src\":\"0x%04X\",\"draw_blend_dst\":\"0x%04X\","
             "\"state_fix\":%d,\"alpha_override\":%.3f}",
             id, enabled, suspended, history, host_hz, target_hz,
             (unsigned long long)swaps, (unsigned long long)d.captures,
             d.w, d.h, d.scale, d.src_x, d.src_y, d.src_w, d.src_h,
             d.source_path, d.force_4_3, d.linear, d.blend_mode,
             d.draw_blend, (unsigned)d.draw_blend_src, (unsigned)d.draw_blend_dst,
             d.state_fix, (double)d.alpha_override);
}
...
/* frame_gate addr=<hex> extra=<0..15> — run a self-capped game's main loop
 * faster by injecting extra ticks into the vblank counter its VSync wait polls.
 *
 * Yu-Gi-Oh! FM caps itself at 30: measured, the wait target advances +2 per
 * loop and every second guest frame issues ZERO draw commands. Its counter is
 * 0x80092AC8 (found via a PC histogram landing in the wait loop at
 * 0x80074310/0x80074358, then the 10952 offset off 0x80090000).
 *
 *   frame_gate addr=0x80092AC8 extra=1     -> loop runs at 60 instead of 30
 *   frame_gate extra=0                     -> off
 *
 * What the result MEANS is the point. This does not touch host pacing, so the
 * emulated machine still runs at real speed: anything clocked off the vblank
 * INTERRUPT (usually the sound driver) should keep correct tempo while the
 * game's loop accelerates. So listen as much as look —
 *   music tempo unchanged + game faster -> loop and audio clocks are separable,
 *     which is what a "duel speed" feature needs;
 *   music speeds up too -> the sound driver rides the main loop and has to be
 *     found and isolated first;
 *   game runs at 2x -> logic is per-iteration, so smoother motion (as opposed
 *     to faster play) additionally needs every per-frame delta halved.
 * Reply reports `ticks` (injections made) and the counter's live value. */
static void handle_frame_gate(int id, const char *json)
{
    char buf[32];
    uint32_t addr = 0; int extra = 0; uint64_t ticks = 0; uint32_t now = 0;
    psx_frame_gate_get(&addr, &extra, &ticks, &now);
    if (json_get_str(json, "addr", buf, sizeof(buf))) addr = hex_to_u32(buf);
    int want = json_get_int(json, "extra", -1);
    if (want >= 0 || json_get_str(json, "addr", buf, sizeof(buf))) {
        psx_frame_gate_set(addr, want >= 0 ? want : extra);
        psx_frame_gate_get(&addr, &extra, &ticks, &now);
    }
    send_fmt("{\"id\":%d,\"ok\":true,\"addr\":\"0x%08X\",\"extra\":%d,"
             "\"ticks\":%llu,\"counter\":%u}",
 ...
 * instruction rather than stretching the vblank period: the CPU fits K times
 * as much work into each frame while every peripheral keeps its cadence.
 *
 * Built to answer whether a title's frame rate is bounded by its own workload.
 * Yu-Gi-Oh! FM renders at 30 (every second guest frame issues zero draws), and
 * driving its VSync counter at 5x changed nothing — so the counter is not the
 * gate. If the workload is, this moves it.
 *
 * Read the result with tools/frame_gate_test.py, whose guest-frames/s figure is
 * the control that must stay ~60: if it drops, host pacing is being affected
 * and the run says nothing about the CPU.
 *
 *   30 -> 60 at normal game speed  : workload-bound AND frame-rate adaptive
 *   30 -> 60 but the game plays 2x : workload-bound, logic is per-iteration
 *   no change                      : not workload-bound either; keep hunting
 */
static void handle_cpu_clock(int id, const char *json)
{
    int mult = json_get_int(json, "mult", -1);
    if (mult >= 0) psx_cpu_overclock_set((uint32_t)mult);
    send_fmt("{\"id\":%d,\"ok\":true,\"mult\":%u}", id, psx_cpu_overclock_get());
}
...
static void handle_screenshot_hires(int id, const char *json)
{
    ...
    /* Pitch is BYTES per row, not pixels — both resolves advance with
     * ((uint8_t *)out + row * out_pitch). Passing the pixel width made each row
     * start a quarter of a row into the previous one, so captures came out as
     * four overlapping copies squeezed into the top of the image. This is the
     * only caller of either resolve, so the bug was invisible everywhere else
     * — and it silently corrupted the one instrument that can see geometry
     * correction at all (the native screenshot resolves 15-bit VRAM). */
    int got = gr_render_display_hires(argb, (int)(ow * 4), (int)di.display_x,
    ...
    if (!got) {
        ...
        scale = 1; ow = w; oh = h;
        got = gr_render_display(argb, (int)(ow * 4), (int)di.display_x,
                                (int)di.display_y, (int)w, (int)h);
...
/* interp_dump — write the frame-interpolation history to PNG.
 *
 * Splits a "the interpolated present is missing a layer" report cleanly, in a
 * single call so all three images come from the same instant:
 *
 *   <prefix>_src.png    the hr FBO re-read LIVE at the rect the last capture
 *                       used — what interp_capture was copying FROM
 *   <prefix>_prev.png   the previous history texture
 *   <prefix>_curr.png   the current history texture
 *   <prefix>_*_a.png    the same images' ALPHA channel as greyscale
 *
 * The alpha planes are the point, not an extra: alpha on this path is the PSX
 * MASK BIT, so if a layer is present in RGB but sits at alpha 0, any blend
 * left armed in the presentation context erases exactly that layer and nothing
 * else. Colour-only dumps cannot see that.
 *
 * src present + history missing -> capture/sync. Both present -> draw/present.
 */
static void handle_interp_dump(int id, const char *json)
{
    GlInterpDebug d;
    gl_renderer_interp_debug(&d);
    if (!d.enabled)          { send_err(id, "interpolation not enabled"); return; }
    if (d.valid < 2)         { send_err(id, "no history pair yet (suspended?)"); return; }
    if (d.w <= 0 || d.h <= 0){ send_err(id, "no capture geometry"); return; }

    char prefix[400];
    if (!json_get_str(json, "prefix", prefix, sizeof(prefix)))
        strncpy(prefix, "psx_interp", sizeof(prefix) - 1);
    prefix[sizeof(prefix) - 1] = '\0';
    const int want_alpha = json_get_int(json, "alpha", 1);

    const size_t npx = (size_t)d.w * (size_t)d.h;
    uint32_t *argb = (uint32_t *)malloc(npx * sizeof(uint32_t));
    uint8_t  *rgb  = (uint8_t *)malloc(npx * 3u);
    if (!argb || !rgb) { free(argb); free(rgb); send_err(id, "alloc failed"); return; }

    static const char *kName[3] = { "prev", "curr", "src" };
    static const int   kWhich[3] = { 0, 1, 2 };
    double alpha_hi_pct[3] = { 0, 0, 0 };
    unsigned long long lum_sum[3] = { 0, 0, 0 };
    int nwritten = 0;

    for (int i = 0; i < 3; i++) {
        if (!gl_renderer_interp_readback(kWhich[i], argb, d.w * 4)) continue;
        size_t hi = 0;
        for (size_t p = 0; p < npx; p++) {
            ...
            if (((v >> 24) & 0xFF) >= 128u) hi++;
            lum_sum[i] += (unsigned)(r + g + b);
        }
        alpha_hi_pct[i] = npx ? (100.0 * (double)hi / (double)npx) : 0.0;

        char path[440];
        snprintf(path, sizeof(path), "%s_%s.png", prefix, kName[i]);
        FILE *f = fopen(path, "wb");
        if (f) {
            if (png_write_rgb(f, rgb, (uint32_t)d.w, (uint32_t)d.h)) nwritten++;
            fclose(f);
        }
        if (!want_alpha) continue;
        for (size_t p = 0; p < npx; p++) {
            uint8_t a = (uint8_t)((argb[p] >> 24) & 0xFF);
            rgb[p * 3 + 0] = a; rgb[p * 3 + 1] = a; rgb[p * 3 + 2] = a;
        }
        snprintf(path, sizeof(path), "%s_%s_a.png", prefix, kName[i]);
        f = fopen(path, "wb");
        if (f) {
            png_write_rgb(f, rgb, (uint32_t)d.w, (uint32_t)d.h);
            fclose(f);
        }
    }
    free(argb);
    free(rgb);
    if (!nwritten) { send_err(id, "readback failed"); return; }

    const double inv = npx ? 1.0 / (3.0 * (double)npx) : 0.0;
    send_fmt("{\"id\":%d,\"ok\":true,\"prefix\":\"%s\",\"files\":%d,"
             "\"width\":%d,\"height\":%d,\"scale\":%d,"
             "\"src_x\":%d,\"src_y\":%d,\"src_w\":%d,\"src_h\":%d,"
             "\"prev_alpha_hi_pct\":%.2f,\"curr_alpha_hi_pct\":%.2f,"
             "\"src_alpha_hi_pct\":%.2f,"
             "\"prev_mean\":%.2f,\"curr_mean\":%.2f,\"src_mean\":%.2f,"
             "\"draw_blend\":%d,\"state_fix\":%d,\"alpha_override\":%.3f}",
             id, prefix, nwritten, d.w, d.h, d.scale,
             d.src_x, d.src_y, d.src_w, d.src_h,
             alpha_hi_pct[0], alpha_hi_pct[1], alpha_hi_pct[2],
             (double)lum_sum[0] * inv, (double)lum_sum[1] * inv,
             (double)lum_sum[2] * inv,
             d.draw_blend, d.state_fix, (double)d.alpha_override);
}
 ...
 *
 * Goes through psx_mod_write_code_word, not write_ram: in a static recomp the
 * instruction is already compiled into the binary, so the address must also be
 * rerouted to the executable-RAM path or the stale compiled form keeps running.
 * Exists to prove a patch can be applied and reverted mid-session, without
 * shipping a restart requirement for a feature that may not need one. */
static void handle_poke_code(int id, const char *json)
{
    ...
    uint32_t addr = hex_to_u32(addr_str);
    uint32_t val  = hex_to_u32(val_str);
    const uint32_t before = psx_read_word(addr);
    psx_mod_write_code_word(addr, val);
    send_fmt("{\"id\":%d,\"ok\":true,\"addr\":\"0x%08X\","
 ...
 * the exact path that delivered it. `count` limits how many entries come back. */
static void handle_savestate_input_trace(int id, const char *json)
{
    extern int psx_savestate_trace_json(char *, unsigned, int);
    int count = json_get_int(json, "count", 64);
    static char body[96 * 1024];
    if (!psx_savestate_trace_json(body, (unsigned)sizeof(body), count)) {
        send_err(id, "trace unavailable");
        return;
    }
    send_fmt("{\"id\":%d,\"ok\":true,%s}", id, body);
}
...
static const CmdEntry s_commands[] = {
    ...
    { "pad_devices",       handle_pad_devices },
    { "osd_toast",         handle_osd_toast },
    { "pad_probe",         handle_pad_probe },
    { "input_lag",         handle_input_lag },
