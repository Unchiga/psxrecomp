...
static void handle_gl_interp(int id, const char *json)
{
     ...
     * call the renderer init uses. */
    {
        ...
        if (enable >= 0 || fps >= 0) {
            ...
            double period = 0.0;
            psx_frame_pacing_state(NULL, NULL, &period);
            gl_renderer_set_interpolation(enable, hh, (double)fps,
                period > 0.0 ? 1000.0 / period : 59.94, d.blend_mode);
        }
    }
...
/* {"cmd":"quit_graceful"}: the File > Quit path. "quit" calls exit() and
 * skips netplay teardown (no BYE, no memory-card carry-back), which is the
 * one thing a netplay test needs to observe. */
extern void psx_runtime_request_quit(void);
static void handle_quit_graceful(int id, const char *json)
{
    (void)json;
    send_ok(id);
