...
GeneratedFunction CodeGenerator::generate_function(
    ...
    if (config_.mod_function_entry_funcs.count(func.start_addr)) {
        body_ss << config_.indent
                << fmt::format(
                       "if (psx_mod_function_entry(cpu, 0x{:08X}u)) {{ "
                       "cpu->pc = cpu->gpr[31]; return; }}"
                       "  /* trusted opt-in game-mod hook/replacement */\n",
                       func.start_addr);
    }
...
void CodeGenerator::emit_runtime_externs(std::ostream& ss) const {
    ...
    ss << "extern int  psx_mod_function_entry(CPUState* cpu, uint32_t address);  /* trusted opt-in game-mod hook/replacement */\n";
